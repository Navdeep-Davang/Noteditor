...
import type {
  ...
  import invariant from '@/dir/shared/invariant';
  ...
  import {$isCodeNode, CodeNode} from '@/dir/nodes/CodeNode';
  ...
  function updateCodeGutter(node: CodeNode, editor: LexicalEditor): void {
    ...
    // Get the previous gutter state from the codeElement, if it exists.
    const previousGutter = codeElement.getAttribute('data-gutter');
    const previousLineCount = previousGutter ? previousGutter.split('\n').length : 0;
  
    // Avoid updating if the line count hasn't changed.
    if (childrenLength === previousLineCount) {
      return;
    }
  
    // Build the new gutter value with line numbers.
    let gutter = '1';
    let count = 1;
